...
def get_prewritten_text(post_data: dict) -> str | None:
    """
    Check if a pre-written text file exists for this post.
    Returns the text content if found, None otherwise.
    """
    # Build the expected filename from image_file
    # e.g., week1_sun_comt.jpg -> week1_sun_comt.txt
    image_file = post_data.get('image_file', '')
    if not image_file:
        return None
    
    text_filename = image_file.replace('.jpg', '.txt').replace('.png', '.txt')
    text_path = config.CONTENT_DIR / "texts" / text_filename
    
    if text_path.exists():
        logger.info(f"Found pre-written text file: {text_path}")
        with open(text_path, 'r') as f:
            return f.read()
    
    logger.info(f"No pre-written text file found at: {text_path}")
    return None


def generate_post_text(post_data: dict, client: anthropic.Anthropic) -> str:
    """
    Get post text - first check for pre-written text file,
    then fall back to Claude generation if not found.
    """
    # First, check for pre-written text
    prewritten = get_prewritten_text(post_data)
    if prewritten:
        logger.info(f"Using pre-written text ({len(prewritten)} characters)")
        return prewritten
